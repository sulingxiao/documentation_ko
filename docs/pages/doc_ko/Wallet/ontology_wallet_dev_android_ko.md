...
sidebar: Wallet_ko
permalink: ontology_wallet_dev_android_ko.html
...
[English](./ontology_wallet_dev_android_en.html) / 한국어
...
Keystore is  a data structure to backup user's account. And it can saved in QR code.Then users can use mobile to scan that QR code to read the data and recover the account. You can check the [Wallet Specification](./Wallet_File_Specification_en.html) for more info.
